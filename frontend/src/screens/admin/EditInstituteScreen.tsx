...
const EditInstituteScreen = () => {
    ...
    const { institute } = route.params as any;
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            await api.put(`/admin/institute/${institute._id}`, formData);
            Alert.alert('Success', 'Institute updated successfully', [
                { text: 'OK', onPress: () => navigation.goBack() }
            ]);
        } catch (error: any) {
            Alert.alert('Error', error.response?.data?.message || 'Failed to update institute');
        } finally {
            setLoading(false);
        }
    };
        ...
        <GradientBackground>
            <ScrollView contentContainerStyle={styles.container}>
                <View style={styles.header}>
                    ...
                    <Text style={styles.title}>Edit Institute</Text>
                    <View style={{ width: 40 }} />
                </View>
...
export default EditInstituteScreen;
